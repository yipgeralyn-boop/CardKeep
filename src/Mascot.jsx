// "Sprout" — the app mascot. Accent-tinted, expression-driven.
// mood: 'idle' | 'happy' | 'cheer'

export function Mascot({ mood = 'idle', size = 120, t, style = {} }) {
  ...
  const sparkle = (cx, cy, r, delay) => (
    <path
      ...
      fill={t.warn}
      style={{ transformOrigin: `${cx}px ${cy}px`, animation: `sparklePop .5s ${delay}s ease both` }}
    />
  );
  ...
  return (
    <div style={{
      ...
      animation: cheer
        ? 'mascotBounce .7s cubic-bezier(.2,1.5,.45,1) both'
        : (mood === 'idle' ? 'mascotIdle 3.4s ease-in-out infinite' : 'none'),
    }}>
