// payoff.jsx — Debt payoff analyzer: projection chart, debt-free date, recommended amount
// Depends on window: money, fmtMonthYear, fmtMonths, blendedApr, simulatePayoff,
//   recommendedPayment, Ic, CardArt, Btn

// Area + line projection chart (balance declining over time)
function PayoffChart({ series, t, monthsLabel, payoffDate }) {
  const W = 320, H = 132, pad = 6;
  const n = series.length;
  const max = Math.max(...series, 1);
  // cap displayed points so very long series stay smooth
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: H, display: 'block', overflow: 'visible' }}>
      ...
      {finite && (
        <g>
          <circle cx={pts[pts.length - 1][0]} cy={pts[pts.length - 1][1]} r="5" fill={t.good} stroke={t.surface} strokeWidth="2.5" />
        </g>
      )}
      {/* start marker */}
...
function PayoffScreen({ t, cards, monthly, setMonthly }) {
  ...
  return (
    <div style={{ padding: '6px 18px 0' }}>
      ...
      {/* recommended callout */}
      {!usingRec && (
        <div style={{ borderRadius: t.radius, padding: 2, marginBottom: 16,
          background: `linear-gradient(135deg, ${t.accent}, ${t.accent}99)`, boxShadow: t.shadow }}>
...
Object.assign(window, { PayoffScreen, PayoffChart });
