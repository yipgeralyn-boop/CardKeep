// reminders.jsx — Upcoming payments timeline + reminder settings
// Depends on window: money, daysUntil, fmtDate, fmtDay, cardStatus, statusColor,
//   Ic, CardArt, StatusPill, Btn
...
function ReminderToggle({ on, t, onToggle }) {
  return (
    <button onClick={onToggle} style={{
      ...
      display: 'flex', justifyContent: on ? 'flex-end' : 'flex-start', transition: 'all .2s', WebkitTapHighlightColor: 'transparent',
    }}>
...
function RemindersScreen({ t, cards, onSelect, reminders, setReminders, tone }) {
  ...
  return (
    <div style={{ padding: '6px 18px 0' }}>
      ...
      <div>
        {upcoming.map((c, i) => <TimelineRow key={c.id} card={c} t={t} onSelect={onSelect} isLast={i === upcoming.length - 1} />)}
      </div>
      ...
      {done.length > 0 && (
        <>
          ...
          <div>
            {done.map((c, i) => <TimelineRow key={c.id} card={c} t={t} onSelect={onSelect} isLast={i === done.length - 1} />)}
          </div>
        </>
      )}
      ...
      <div style={{ background: t.surface, borderRadius: t.radius, boxShadow: t.shadow, overflow: 'hidden' }}>
        {settings.map((s, i) => (
          <div key={s[1]} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 16px',
            borderBottom: i < settings.length - 1 ? `1px solid ${t.line}` : 'none' }}>
...
Object.assign(window, { RemindersScreen, ReminderToggle, TimelineRow });
