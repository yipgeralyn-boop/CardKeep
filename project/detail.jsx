// detail.jsx — Card detail screen + Pay sheet + Paid celebration
...
function DetailScreen({ t, card, onBack, onPay }) {
  ...
  return (
    <div style={{ padding: '4px 18px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        background: sc.bg, borderRadius: t.radius, padding: '14px 16px', marginBottom: 16 }}>
      ...
      <div style={{ background: t.surface, borderRadius: t.radius, boxShadow: t.shadow, overflow: 'hidden' }}>
        {[
          ...
          <div key={row[0]} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 16px',
...
// ─── Pay sheet ────────────────────────────────────────────────
function PaySheet({ t, card, onClose, onConfirm }) {
  const [amt, setAmt] = React.useState('full'); // 'min' | 'full' | 'custom'
  const [custom, setCustom] = React.useState('');
  const value = amt === 'min' ? card.min : amt === 'full' ? card.statement : (parseFloat(custom) || 0);

  const Opt = ({ id, label, sub, val }) => {
    const active = amt === id;
    return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
        ...
        {amt === 'custom' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: t.surface2, borderRadius: 16, padding: '14px 16px', border: `1.5px solid ${t.accent}` }}>
            ...
            <input autoFocus value={custom} onChange={(e) => setCustom(e.target.value.replace(/[^0-9.]/g, ''))} placeholder="0.00" inputMode="decimal"
...
function PaidSheet({ t, card, amount, onClose }) {
  const msg = React.useMemo(() => CHEERS[Math.floor(Math.random() * CHEERS.length)], []);
  return (
    <SheetShell t={t} onClose={onClose} title="" compact>
      <div style={{ textAlign: 'center', padding: '8px 8px 4px' }}>
        ...
        <div style={{
          display: 'inline-block', background: t.accent, color: t.onAccent, fontFamily: 'var(--font-ui)',
          fontWeight: 700, fontSize: 15, padding: '9px 16px', borderRadius: 16, position: 'relative',
...
// ─── Bottom sheet shell ───────────────────────────────────────
function SheetShell({ t, children, onClose, title, compact }) {
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', animation: 'fade .25s ease' }} />
      <div style={{ position: 'relative', background: t.surface, borderTopLeftRadius: 30, borderTopRightRadius: 30,
        padding: '12px 22px 38px', animation: 'slideUp .32s cubic-bezier(.2,.9,.3,1)', boxShadow: '0 -10px 40px rgba(0,0,0,0.2)' }}>
        <div style={{ width: 38, height: 5, borderRadius: 999, background: t.line, margin: '0 auto 16px' }} />
        {title ? <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 21, color: t.text, marginBottom: 18 }}>{title}</div> : null}
        {children}
      </div>
    </div>
  );
...
Object.assign(window, { DetailScreen, PaySheet, PaidSheet, SheetShell });
